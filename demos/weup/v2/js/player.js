var Framer = {
  ...
  init: function (scene) {
    this.canvas = document.querySelector('canvas#player');
    this.scene = scene;
  ...
  configure: function () {
    ...
    this.countTicks = 270 * Scene.scaleCoef;
  },
  ...
  drawTick: function (x1, y1, x2, y2) {
    ...
    gradient.addColorStop(0, '#6662E1');
    gradient.addColorStop(0.6, '#6662E1');
    gradient.addColorStop(1, '#6662E1');
    this.context.beginPath();
...
var Tracker = {
  ...
  drawArc: function () {
    this.context.save();
    this.context.strokeStyle = 'rgba(5, 207, 112, 1)';
    this.context.beginPath();
...
var Scene = {
  ...
  canvasConfigure: function () {
    ...
    this.context.strokeStyle = '#6662E1';
    this.calculateSize();
  },

  calculateSize: function () {
    this.scaleCoef = Math.max(0.5, setMinSize / this.optimiseHeight);
...
var Controls = {
  ...
  drawPic: function () {
    this.context.save();
    this.context.beginPath();
    this.context.fillStyle = 'rgba(0, 16, 47, 1)';
    this.context.strokeStyle = 'rgba(5, 207, 122, 1)';
    this.context.lineWidth = 1;
    ...
    this.context.arc(this.scene.radius + this.scene.padding + x, this.scene.radius + this.scene.padding + y, 3, 0, Math.PI * 2, false);
    this.context.fill();
    this.context.stroke();
    this.context.restore();
  },
...
var Player = {
  ...
  tracks: [
    {
      artist: "Mission Impossible 6",
      song: "Lorne Balfe",
      url: "tracks/mission-impossible.mp3"
    },
    {
      artist: "And-Man and The Wasp",
      song: "Christophe Beck",
      url: "tracks/ant-man-and-the-wasp.mp3"
    },
    {
      artist: "Dunkirk",
      song: "Hans Zimmer",
      url: "tracks/dunkirk.mp3"
    },
  ],
  ...
  loadTrack: function (index) {
    ...
    request.onload = function() {
      that.context.decodeAudioData(request.response, function(buffer) {
        ...
        that.gainNode.connect(that.destination);
      });
    };
