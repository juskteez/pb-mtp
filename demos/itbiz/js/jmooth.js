...
function jmooth(target) {

  // SETTINGS
  var scroll_Area_id   = target,
  ...
  if (scroll_Content) {
    ...
    addEvent(window, 'load', window.windowLoad('by load'), 0);
    ...
    //if (hoverScroll) dragging = true;

    document.addEventListener('mousemove', function(event) {
      if (loaded && (dragging || scroll_Bar_Dragging) && hoverScroll == false) {
        event.preventDefault();
      ...
      if (loaded && hoverScroll) {
        //console.log('hello');
        event.preventDefault();

        scroll_Proceeding();

        (horizontal) ? posX = window_Width - event.clientX : posY = event.clientY;

        dragger(posX,posY,(window.innerWidth/2),drag_Speed);
    ...
    var action_range = document.getElementById('articles_page');

    action_range.addEventListener('touchstart', function(event) {
      if (loaded && window_Width > 600) {
        event.preventDefault();
    ...
    action_range.addEventListener('touchmove', function(event) {
      touch_Object = event.changedTouches[0];

      if (loaded && touching && window_Width > 600) {
        event.preventDefault();
        scroll_Proceeding();
    ...
    action_range.addEventListener('touchend', function() {
      cancel_Touch();
    }, {passive: false});

    action_range.addEventListener('touchcancel', function() {
      cancel_Touch();
    }, {passive: false});
